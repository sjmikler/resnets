...
def regularized_padded_conv3(*args, **kwargs):
    return tf.keras.layers.Conv2D(*args, **kwargs, kernel_size=3, kernel_regularizer=_regularizer, padding='same')
...
def VGG(input_shape, n_classes, weight_decay=0, group_sizes=(1, 1, 2, 2, 2),
    ...
    output = tf.keras.layers.Dense(n_classes, kernel_regularizer=_regularizer)(bn_relu(pooled))
